-- =====================================================================
-- Relatti Spine — fix: derive engagement/partner status from FACTS
-- =====================================================================
-- APPLIED TO PRODUCTION 2026-07-01.
--
-- Bug: an invitee who signed up via the link BEFORE finishing their assessment
-- (the normal invitee path) got permanently stuck. The claim (claimPendingInvites)
-- is the only writer that advances decoded_invites.status 'pending' → 'completed',
-- and it only fires on the FIRST claim (recipient_id IS NULL) *and* only if the
-- recipient already has a report. Sign-up-before-completing therefore locks the
-- invite at status='pending'. relatti_sync_invite then computed
-- partner_status='invited' from that stale column, freezing the recipient's
-- participant at 'invited' — which getRelationships() filters out (status IN
-- active/consented), so the invitee's dashboard showed "Invite your partner"
-- while the inviter saw a live, connected dyad. (Asymmetric-connection bug.)
--
-- Fix: "connected" is defined by two facts — the recipient has claimed an account
-- (recipient_id) AND completed their own profile (recipient_report_id) — not by
-- the fragile decoded_invites.status text column (which has several writers and
-- drifts). Deriving both eng_status and partner_status from those facts makes the
-- spine self-correct on the next sync regardless of what status says.
--
-- Everything else in the function is byte-for-byte identical to
-- 20260616201208_relatti_e3_sync_invite_fn.sql.
...
BEGIN
  ...
  -- "Connected" = the recipient has BOTH claimed an account (recipient_id) AND
  -- completed their own profile (recipient_report_id). Derive status from those
  -- facts, NOT from decoded_invites.status — that column only advances to
  -- 'completed' on the recipient's first claim, so a recipient who signs up
  -- before finishing their assessment leaves it stuck at 'pending' forever.
  IF inv.recipient_id IS NOT NULL AND inv.recipient_report_id IS NOT NULL THEN
    eng_status := 'active';
