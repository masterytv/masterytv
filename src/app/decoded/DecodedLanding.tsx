 ...
 * Google OAuth + Magic Link, same pattern as coachapp login.
 */
export default function DecodedLanding() {
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: `${window.location.origin}/decoded/auth/callback`,
      },
    });
  ...
  async function handleGoogleLogin() {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/decoded/auth/callback`,
      },
    });
        ...
        {sent ? (
          ...
          >
            ...
            <p className="mt-2 text-sm text-text-secondary">
              ...
              <br />Click the link to start your assessment.
            </p>
            <button
