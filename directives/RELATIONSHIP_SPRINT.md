...
>
> **STATUS — ✅ SHIPPED + PUSHED to staging (2026-06-29, commits 210c1a3 + 5c81c77).** E14.1 (research/stance spec → `RELATTI_EXPERIENCE.md §5.6.1`, founder-approved) + E14.3 (rebuild) done, plus three root-cause fixes the spec didn't anticipate: (a) the relationship coach inherited the *executive* scaffolding → skip Layers 2/3/6/8/9, swap Layer 10 (`buildRelationshipGuardrails`), harden Layer 11; (b) **the coach ran on gpt-4o-mini, not Claude** (`callClaudeStreaming` is OpenAI-primary; gpt-4o-mini missed a suicidal-ideation cue Claude caught) → added `forceClaude`, relationship coach now runs **claude-sonnet-4-6**; (c) history was engagement-scoped → `assemblePrompt` scopes to `conversationId` for relationship. Persona finalized to **understand-first + human voice**; guardrails seed professional support once on sustained grief/depression. **Validated: a 9/10 test session** (relatti7 conv `f0ca709b`). **Tool:** `scripts/coach-lab/run.mjs` (local prompt lab — A/B across models+history, safety battery). **Remaining (optional, not blocking):** E14.2 gold-example snapshot tests, E14.4 deeper per-style calibration, E14.5 "just listen / give me something to try" mode toggle.
...
### E15 — Trust, Privacy & Legal Launch-Readiness  🔒⚖️  *(added 2026-07-01, founder)*
*Goal: Relatti stores extraordinarily sensitive data (DV, child-safety, grief, mental-health, infidelity) plus non-consenting third parties (partners, children), and the coach currently makes confidentiality promises the system doesn't keep. Make the promises true, build the legal/privacy document set, and stand up a safety + child-safety escalation protocol before any public launch. Surfaced by the relatti21 session (`85950a7f`, 78 msgs) — full ranked assessment in `directives/PRIVACY_TERMS_LIABILITY_PLAN.md`. **Absorbs the earlier clinician-review launch blocker; relates to E7.***
> **HARD LAUNCH BLOCKER. NOT legal advice** — requires a privacy/health-tech attorney + an AI-therapy-law specialist + a licensed clinician.

| Story | Done |
|:--|:--|
| **E15.1** 🔴 Coach confidentiality-script fix (lawyer-independent — do FIRST). Rewrite the relationship guardrails + safety layer so the coach NEVER promises "I don't report to anyone / I don't share what you tell me / you're safe to talk here." Honest version: private-from-partner (guaranteed in code) + processed/stored by the company + safety concerns may be reviewed + link the privacy policy. | ✅ **DONE 2026-07-01.** Rewrote `prompt-assembler.ts` relationship guardrails + shared safety layer; mirrored into coach-lab (`lib.mjs`/`run.mjs`); added `privacy-confidential` scenario + HARD `noAbsoluteConfidentiality` assertion; GATE PASSED; coach redeployed. 🧪 |
| **E15.2** LLM data handling — the question the user literally asked (n77). Confirm + configure **zero-retention / no-training** with OpenAI + Anthropic; sign DPAs. | 🟡 **RESEARCHED 2026-07-01 → founder action.** Both providers already **default to no-training on API/commercial traffic** + ~30-day retention + DPAs available (Anthropic's is auto-incorporated into Commercial Terms; OpenAI executable). Action checklist + the OpenAI **NYT litigation-hold caveat** (undercuts a clean 30-day-deletion claim): `directives/LLM_DATA_HANDLING.md`. Privacy Policy §4 states the no-train position behind a gate comment. Remaining = founder signs DPAs + requests ZDR. |
| **E15.3** Make isolation real: a **partner-isolation regression test** (coaching content is `user_id`-scoped, never leaks to the partner), admin-access **audit logging** on `crisis_flags`/messages, verify `delete-user-data` covers every content table. | ✅ **DONE 2026-07-01.** (1) Isolation test DONE — `isolation-check.mjs` (gated, 5/5 + negative-control); RLS verified live. (2) **Audit logging DONE + deployed** — `admin_audit_log` (migration `20260701140000`, RLS default-deny, excluded from delete-user-data) + `admin-data` logs sensitive reads/writes. (3) **`delete-user-data` REWRITTEN + DEPLOYED (v8)** — now covers all ~35 user-linked tables, FK-order verified via a rolled-back `DO`-probe on live data. Couples policy (founder call): remove ALL compatibility data, **preserve the partner** (detach not destroy — keep their engagement/conversations), email the partner; delete-confirm modal warns (Relatti-gated). 🧪 |
| **E15.4** Safety-escalation + **child-safety / DV protocol**: wire **Tier-1 high-severity → escalation email** (gap found 2026-07-01 — only Tier-2 emails today); define detection → human-review path → what we do and explicitly do NOT do; document it. | ✅ **DONE 2026-07-01.** Both tiers now escalate via shared `sendSafetyEscalationEmail` (Tier-1 via `waitUntil`, deduped 12h per user/conv/category); Tier-1 flags now conversation-scoped; verified 13/13 offline; protocol at `directives/SAFETY_ESCALATION_PROTOCOL.md`; coach redeployed. (Fixed a latent `channel-router` ReferenceError in passing.) |
| **E15.5** Draft the document set (Claude drafts → attorney reviews): Privacy Policy, ToS, AI/Coaching Disclaimer, Safety & Escalation Policy, Retention/Deletion, Couples Data-Sharing & Consent, Subpoena/Law-Enforcement Response. Gate signup on acceptance. | ✅ **DONE 2026-07-01 (attorney-review-pending drafts).** Brand-aware `(legal)` route now branches on brand: Relatti Privacy / Terms / Disclaimer at `src/app/(legal)/_content/Relatti*.tsx` (the other 4 docs folded in as Privacy sections — safety-escalation, retention/deletion, couples-sharing, subpoena). New `/disclaimer` route; layout + footer brand-aware. Signup **acceptance-gated** in `LoginPanel.tsx` (checkbox blocks email + Google signup; version recorded in user metadata — email via `signUp` data, OAuth via `legal_ack` cookie reconciled in `auth/callback`). Shared version in `src/lib/platform/legal.ts` (`LEGAL_VERSION`). tsc clean; both brands verified in preview. **DO NOT publish/launch before E15.6.** Founder to-do: create `privacy@/support@relatti.com` inboxes. |
| **E15.6** Engage + incorporate review: privacy/health-tech attorney, AI-therapy-law specialist (IL WOPR / NV AB406 / UT), licensed couples/crisis clinician. | Professional sign-off folded into docs + protocol. |

**E15 exit:** the coach's promises match reality; LLM no-train confirmed; partner isolation tested; child-safety/DV escalation protocol live; Privacy + Terms + Disclaimer published and acceptance-gated; attorney + clinician sign-off. **No public launch (beyond the Vercel-protected beta) until at least E15.1, E15.2, E15.3 + published Privacy + Terms.**
