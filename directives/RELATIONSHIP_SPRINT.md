...
| **E15.2** LLM data handling — the question the user literally asked (n77). Confirm + configure **zero-retention / no-training** with OpenAI + Anthropic; sign DPAs. | Written confirmation + config; privacy policy can honestly say "not used to train third-party models." |
| **E15.3** Make isolation real: a **partner-isolation regression test** (coaching content is `user_id`-scoped, never leaks to the partner), admin-access **audit logging** on `crisis_flags`/messages, verify `delete-user-data` covers every content table. | 🟡 **PARTIAL 2026-07-01.** Isolation regression test DONE — `scripts/coach-lab/isolation-check.mjs` (gated in `test.mjs`, 5/5 + negative-control) guards the service-role path; RLS `auth.uid()=user_id` verified live on all content tables. **Remaining:** admin audit logging + `delete-user-data` coverage. 🧪 |
...
| **E15.5** Draft the document set (Claude drafts → attorney reviews): Privacy Policy, ToS, AI/Coaching Disclaimer, Safety & Escalation Policy, Retention/Deletion, Couples Data-Sharing & Consent, Subpoena/Law-Enforcement Response. Gate signup on acceptance. | Drafts exist; signup acceptance gate; attorney-review-ready. |
