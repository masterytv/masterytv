...
| **E15.3** Make isolation real: a **partner-isolation regression test** (coaching content is `user_id`-scoped, never leaks to the partner), admin-access **audit logging** on `crisis_flags`/messages, verify `delete-user-data` covers every content table. | Isolation test in CI; audit log; deletion verified. 🧪 |
| **E15.4** Safety-escalation + **child-safety / DV protocol**: wire **Tier-1 high-severity → escalation email** (gap found 2026-07-01 — only Tier-2 emails today); define detection → human-review path → what we do and explicitly do NOT do; document it. | Tier-1 escalates; written protocol; ties to E7 + clinician review. |
