...
- [ ] 🔥 **The program axis is CHEAP; the brand axis is EXPENSIVE. Do them separately, in that order** (learned on `integration`, 2026-08-11). Adding a `ProgramId` produced **five** bounded `Record<ProgramId,…>` errors and landed in an afternoon. Adding a `BrandId` compile-errors **nine** `Record<BrandId,…>` files at once — OG palette, sitemap, login panel, legal, notify sender, invite email chrome, middleware, brand-metadata — and `check:brand-tokens` additionally demands a full identity token set in **both** themes. Every one of those is an identity decision, so doing it before the wordmark is cleared means doing §5.1 blind and then doing it again. **Corollary: a vertical can be fully built on the program axis while its brand is still undecided** — the coach, the safety kernel, the memory taxonomy and the battery are all program-keyed. What genuinely waits for the name is chrome: the dashboard/sidebar/topbar/middleware branches are **brand**-keyed, so they cannot be written at all until the `BrandId` exists.
- [ ] **A registered program with no pack must be `null`, not a stub.** `PACKS[slug] = null` + a throwing `resolvePack` keeps the loud failure an unregistered program used to get, while the exhaustive Record still forces the line to be written by hand. A placeholder pack resolves, speaks, and writes memory in whatever voice the placeholder happened to carry — which is worse than the error it was added to avoid.
...
- [ ] **The public name is trademark-cleared BEFORE it enters any copy, prompt, or email** 🔥 (the money vertical shipped weeks of surfaces + coach prompts under "Money Maps™", then hit a third party's registered mark and needed a ~60-file rename + full edge redeploy, 07-20 — see [MONEY_TRAITS_RENAME.md](MONEY_TRAITS_RENAME.md)). Write the **naming contract** on day one: the display wordmark is CHANGEABLE (strings only); storage identifiers (instrument ids, JSONB section keys, the program slug) are LOCKED the moment real rows exist and keep their spelling through any rebrand. If a name is ever banned, add it to `scripts/check-banned-brand-terms.mjs` (`check:brand-terms`) + BRAND.md §1.1 so it can't return by accident.
