...
> 3. **The homepage was REPLACED with the Cowork deck verbatim** (`MoneyLanding.tsx` + page metadata). ✅ The leak-vs-Challenge tension is RESOLVED — founder call same day: "**Use Challenge vs Leak**" → the deck's "leak" lines render as "challenge" on both landing pages, matching the in-product card/report/coach vocabulary.
> 4. ✅ **"The Money Tell" LP is BUILT at `/tell`** (founder delivered the poker-frame deck 2026-07-20; implemented verbatim with the Challenge swap; `TellLanding.tsx` + §15 metadata, noindex; the layout head-script treats `/tell` as a money path so the emerald brand resolves on any host). At go-public, add `/tell` to sitemap/robots alongside `/money`.
