...
| **PC2.1h** 🔨 **DESIGNED, not built** — [INVITE_PROGRAM_DESIGN.md](INVITE_PROGRAM_DESIGN.md). Invite surfaces (`invite/route.ts:48,93`, `invite-consent:173`, `invite-notify:73`, `invite/[code]:98`, `claim-invites.ts:62,133`) still take latest-report-by-user_id. | A Relatti invite carries the Relatti report. |

### 6.1 Why PC2.1h is deferred (read before picking it up)

> 🔴 **SUPERSEDED 2026-07-16 — the risk assessment below was WRONG.** Design done: **[INVITE_PROGRAM_DESIGN.md](INVITE_PROGRAM_DESIGN.md)**. "Low risk, Tom has 0 invites" was based on a query that filtered out broadcast rows. **He has a broadcast invite carrying his general report**, and `getOrCreateBroadcastInviteUrl` upserts on `(inviter_id, recipient_email)` — one row per user across ALL brands. So the moment he loads his Relatti dashboard after taking the battery, it **overwrites his Decoded share link in place**. PC2.1a–g made that reachable. The fix is a unique-constraint change, not just a column. The rest of §6.1 stands as the reason it wasn't done *inside* PC2.1.
