...
| **PA4.2** Gate route + nav rendering for `commitments`/`progress`/`coaching-letter`/`compatibility` on module flags. ✅ *Route half completed 2026-07-15 (`52bbc02`): `ROUTE_MODULES` map + middleware guard — a gated path whose module is off for the resolved brand redirects to `/dashboard` (nav hiding alone was not brand isolation). Also: `coach_voices` module (general-only; six executive personas gated off relatti chat) + `beta_program` (relationship-only; `/dashboard/beta` guarded off masterytv).* | MasteryTV = all on (unchanged); a program with them off hides them. 🧪 |
...
| **PC4.3** ✅ *(shipped 2026-07-14 eve)* Pack-owned post-processing: `CoachPack.extraction` (`PackExtraction`: factCategories · factsRule · aiToolsRule · extractAiTools · frameworkChallenges) in packs/types.ts; post-processor resolves the pack and composes its extraction prompt + side-effect gates from it — zero domain ternaries left there. Executive extraction text verbatim-preserved. Relationship taxonomy: `theme` / `attachment_cue` + shared categories, business/org facts banned, AI-tool harvesting off, framework challenges off. Migration `20260714220000` extends `memory_facts_category_check` (live-only constraint, same history-gap class); extractor output clamped to the pack's category set so one rogue category can't kill the batch insert; Memory-page labels added. **Verified:** relationship spot-check produced `[pattern] pursue/withdraw`, `[attachment_cue] fear of abandonment`, `[theme]` and an EMPTY ai_tools array despite the user naming Slack/Figma/an agency; PC6.1 replay re-run PASSES on the parameterized prompt; goldens 4/4 byte-identical. | Relationship convs produce relationship-shaped memory. ✅ |
| **PC4.4** ✅ *(shipped 2026-07-15)* Spine-based pack resolution (audit Phase 4) 🔒: `resolveProgram` precedence is now **named engagement (membership-verified) > participant membership (engagement.kind/program slug — outranks the client hint, so a stripped/forged "general" can't hand a dyad member the executive persona) > validated hint > `users.signup_brand` (PC5.2 stamp — closes the solo-user silent-executive path) > legacy invite heuristic > explicit null (junk client strings never leak into `metadata.program`)**. New gate: `npm run check:resolve` (`scripts/coach-lab/resolve-program-check.ts` — real `resolveProgram()` against a count-aware fake PostgREST, 9 precedence cases incl. the founder shape: invite SENDER + "general" hint keeps the executive coach) wired into `npm run gate` + CI. Founder account stamped `signup_brand='masterytv'` (data fix) so the invite heuristic can't flip his email replies. Deployed: coach v90, email-inbound v34, telegram-webhook v30. Goldens 4/4 byte-identical. | No silent executive fallback path remains. ✅ 9/9 precedence cases in the gate. |
...
| **PC4.6** ✅ *(shipped 2026-07-15, `5f8b8b3`)* Pack-authored proactive briefings: `CoachPack.briefing` (enabled · system · buildPrompt · subject · fallback · metaCheckin); cron-morning-briefings is vertical-blind (resolve pack → compose → deliver). Executive text verbatim-preserved (proven byte-identical offline). Relationship copy = a small daily moment of attention toward the relationship, **LOW-DISCLOSURE** (email is lock-screen-visible → never references session content; subject is always just a greeting — the 92d221d invariant). `enabled:false` turns a vertical's proactive touch off in one line. Deployed coach v94 / test-guardrails v21 / email-inbound v37 / telegram-webhook v33 / cron-morning-briefings v14. | Relationship users get relationship-voiced briefings; executive briefings byte-identical; goldens 4/4. ✅ |
...
| Story | Done |
|:--|:--|
| **PC6.1** ✅ *(shipped 2026-07-14 eve)* Extractor supersede rule + `superseded` status + backstop, per design above. Implementation: `buildExtractionPrompt()` extracted from post-processor (exported so the replay harness runs the byte-identical production prompt) and fed the conversation's active commitments (join via `source_message_id → messages`); per-row inserts honor `supersedes` (only ids actually offered to the model); embedding backstop (`SUPERSEDE_SIMILARITY=0.86`, 30-min window, `cosineSim` in code). Migration `20260714210000`: `superseded_by` column + **the live `commitments_status_check` constraint extended** (it exists in the cloud but NOT in the committed baseline — another 44-vs-7 history gap; first repair attempt violated it). All `status='active'` readers (4 crons, dashboard, progress) exclude superseded rows with zero changes — audited each one. **Acceptance PASSED:** `scripts/coach-lab/replay-supersede.ts` replays the real 7/13 transcript in memory (zero DB writes) → extractor chained c1→c2→c3→c4 supersedes → exactly ONE active; backstop never needed. Live 7/13 rows repaired (2 stale actives → superseded_by the completed brother commitment). | Replaying the 7/13 19:36–19:40 transcript yields ONE active commitment; check-ins never reference a superseded row. ✅ |

#### PC7 — Chat-first zero-friction entry  ⏸️ **PARKED — do not plan yet**  *(added 2026-07-17, founder-directed)*
*Goal: collapse the value wall. Today a visitor must (1) take a 10–30-min assessment, (2) accept privacy/terms, (3) give an email, and (4) still gets little value until they chat. Founder direction: let them **chat with the coach immediately** — the coach itself earns signup, assessment, goals, progress, and invites progressively, so people experience that this is more than a chatbot instead of reading it. Platform-wide (every vertical's funnel).*

> **⏸️ Founder instruction (2026-07-17): research gathered, but do NOT plan or build this until the money-vs-career next-vertical decision is made** (see [MONEY_DISCOVERY.md](MONEY_DISCOVERY.md)). Full research findings + the progressive-gate ladder sketch live in **MONEY_DISCOVERY.md Appendix A**. Headlines: ChatGPT's account-less tier gates memory behind signup (the natural hook for a memory-based coach); form/assessment friction roughly halves conversion; reverse-trial funnels benchmark 4–12% free→paid; Cleo reports 50% free-to-paid in our exact category; counterweights = LLM cost on majority-bot anonymous traffic (57.5% of requests are automated), lead quality, consent gating for anything sensitive, crisis kernel on anonymous chats. Related prior signal: the 6-model study's "the assessment is a conversion tax, not a moat" (`STRATEGY.md` §2).

| Story | Done |
|:--|:--|
| **PC7.0** ⏸️ Blocked on the next-vertical decision; then scope as its own epic (anonymous coach endpoint, rate-limits/abuse, session→account claiming, progressive assessment-in-chat, consent-gate placement). | Not before the MONEY_DISCOVERY.md gate resolves. |
...
| Story | Done |
|:--|:--|
| **PV1.1** ✅ *(shipped 2026-07-15, founder-directed after the tester1/tester2 incidents)* Formalize `VERTICAL_PLAYBOOK.md` from seed → standard: Phase 0.5 process (§2–§4) + a **§5 mechanical Launch Checklist** — brand · SEO/AEO · assessment · coach pack · dashboard/modules · consent/privacy · proactive/channels · admin · infra/DB, each 🔥 item tied to the real Relatti incident it prevents, plus a §5.10 8-step verification sweep (dual-brand pass, crawler pass, proactive pass, consent pass, coach-context pass). Self-annealing rule: every cross-vertical bug adds its prevention to §5 **in the same session as the fix**. | A repeatable SOP any vertical can follow. ✅ |
| **PV1.2** ✅ *(shipped 2026-07-15)* Gate wired into the methodology: `CLAUDE.md §8` gained a "New vertical/domain → Playbook is MANDATORY" block (Experience Discovery first, §5 checklist, §5.10 sweep, advisory skip-gate) + `PLATFORM_ARCHITECTURE.md` header cross-link ("config, not codebases" is the architecture; the playbook is how you exercise every config point). | The gate is discoverable where work starts. ✅ |
| **PV1.3** First application — run Phase 0.5 for the **next** vertical *before* building it. ⚠️ *On hold 2026-07-17: which vertical is next (career vs **money**) is an open founder gate — see [MONEY_DISCOVERY.md](MONEY_DISCOVERY.md). If career: `CAREER_EXPERIENCE.md` (job-transition psychology; hero = the job search/deadline; battery = personality + RIASEC). If money: `MONEY_EXPERIENCE.md` (per MONEY_DISCOVERY.md §9).* | The next vertical starts from an experience spec, not a re-theme. |
