...
| Story | Done |
|:--|:--|
| **PC5.1** Nav grouping + scope labels: group into **Platform** (Cost Dashboard, Crisis Flags, User Management) / **MasteryTV · Executive** (Frameworks) / **Relatti** (Beta Cockpit); every vertical-scoped page gets a plain-language scope banner (Beta Cockpit: "Relatti beta program only — MasteryTV has no beta funnel"). Copy-only, ships first. | No admin page leaves its scope ambiguous. |
| **PC5.2** User Management brand enrollment: per-user **brand chips** + view/sort/**hide** by brand. Derivation today: relationship = `participant`/`decoded_invites` rows; executive = assessments/coach threads without a relationship engagement. Durable fix going forward: stamp **signup brand** on the user row at creation (resolved brand is already on every request as `x-brand`). | Can answer "which users are Relatti vs MasteryTV" from the table; brands sortable/hideable. |
| **PC5.3** Frameworks page made self-explanatory: explainer block stating what the library actually does — the post-processor auto-assigns a framework to each newly detected challenge (`assignFramework()`: category → default framework, gated by user trust tier); **Active** removes a framework from that auto-assignment pool; **Weight** biases selection. Banner: "Executive (MasteryTV) engine only — Relatti runs stance-based coaching with **no framework library, by design** (COACH_ARCHITECTURE_AUDIT §8)." Add a program/vertical column so a future Relatti or career library slots in visibly (rides PC4.3 / audit Phase 3 domain-scoping of `framework_config`). | A new operator can read the page and know exactly what toggling/weighting does and which vertical it affects. |
| **PC5.4** Crisis Flags brand attribution 🔒: stamp the **resolved program** on each `crisis_flags` row at write time (the kernel knows it at detection); view gains brand filter/sort. Safety visibility stays UNIFIED (one list, filterable — never split per admin). | Flags attributable + sortable per brand; zero-flag state explains what would appear here and from which verticals. |
| **PC5.5** Cost Dashboard per-brand breakdown 💵: stamp `program` on `cost_tracking` writes (coach, post-processor, embeddings, report generation); dashboard shows **per-brand columns + platform totals**; backfill best-effort (conversation→engagement joins), remainder labeled "unattributed". | Costs read per-brand at a glance and sum to the existing totals. |
