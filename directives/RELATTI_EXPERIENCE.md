...
- **✅ Both 2026-08-05 copy-audit gaps now CLOSED (same day):**
  - **Step 2 is specific.** It named neither what the partner receives nor their time cost, on the one step the funnel actually loses. It now says both, using the SHIPPED facts: *"They get an email saying you invited them to understand your relationship together. Ten minutes on their side, then you're linked, privately and by consent."* The first half paraphrases `invite-email.ts` → `relatti.subject`; the ten minutes is what that same email tells the partner. **If either changes in `invite-email.ts`, change this line too** — it is the only place the site promises what lands in someone else's inbox.
  - **The #1 objection is answered.** A two-question block sits directly above the final CTA (`OBJECTIONS` in `RelattiLanding.tsx`): *"My partner won't do this."* → start anyway, the solo path is the full product (§5.6's "never gate coaching or shame the gap", stated to the visitor for the first time); and *"Won't sending it feel like an accusation?"* → what they open is a description of the sender. That second answer is the hero-is-the-invite rule made explicit for the reader, which is why it belongs on this surface and not in the app. Two questions, not three: the page already spends its rule-of-three budget on the pillars and the steps. Plain prose, no icon tiles, so one template on a page of card grids is broken on purpose (BRAND.md §14.6). Skipped on `/samefight`.
- **Beta-gate honesty (2026-08-05):** the hero trust line read *"Free to start · about 10 minutes · no card required"* while all three start CTAs route to `/beta`, where `BetaOffer` gates submit on a valid invite code — a bait read for cold traffic. Now *"Free during the beta · about 10 minutes · invite code required"*, naming the requirement before the click rather than after. **Revert both this line and the `/beta` → `/assess` CTA switch together at public launch**; the CTA labels themselves stay verbatim-matched to the H1.
- Principles: 1, 2.
...
- **Target — restructure to relationship-first:**
  ...
  6. ⭐ **Relational interpretation per section (founder, 2026-06-26):** every section carries a **"What This Means for You, Their Partner"** sub-block that translates this person's result — and, once shared, the partner's — into *what it means for the two of you together*. The profile reads as dyad-interpretive, not two solo reports. Governed by consent; **private coaching is never surfaced.** State the privacy rule plainly in the share UI.
     - ✅ **Half 1 — "what each of you needs to hear" (built 2026-07-16).** Lives in the **compatibility report**, not the solo profile (founder, 2026-07-16): the phrases only exist once BOTH partners have finished, so a section in the *solo* profile was a permanently empty promise for anyone un-partnered. (It had literally shipped that way — `phrases={[]}` hardcoded.) The compat report by definition exists only after both complete, so it can never be empty there. Both sides render **side by side** — most couples need opposite things, and that contrast *is* the insight. The solo profile keeps a one-line **pointer** (`PartnerPhrasesPointer`) that routes to the pair when ready and to the invite ask when not. Resolved by `getDyadNeedToHear` in `src/lib/relatti/partner-need-to-hear.ts`; computed in **code, not the LLM**, per the dyad-interpretive rule (two independent renders would contradict). No report regeneration needed; retakes propagate free (follows `decoded_invites.{inviter,recipient}_report_id`).
       - ⛔ **COPY RULE (founder, 2026-07-16 — do not regress):** these phrases are **generated by the model from a person's assessment results**. Nobody wrote them. Copy must **NEVER** imply they're the partner's own words, or anything the partner wrote/told us — we promise a partner's private coaching is never shared, and that wording undermines the promise even though the data is harmless. Name the SOURCE: *"Based on their relationship style — the words most likely to land."* The original *"in their own words, from their profile"* was factually false **and** privacy-alarming; the same false claim was also live in `PartnerInviteModal` ("in their own words") — the very surface asking someone to invite their partner — and is fixed. (The ritual's "see what they said" is *different and true* — partners really do write those.)
       - **Client-payload rule:** `ReportViewer` is a client component, so the solo page passes only a `DyadPointer` (`toDyadPointer`) — handing it the full object serialized BOTH partners' phrases into the payload of a page that renders neither.
       - **Consent: gated at `share_with_human = 'full'`** — the same rung `verifySharedAccess` uses to let a partner open the *whole* report page, so this discloses **nothing new**; it only puts already-readable data where it's useful. The default invite-link flow auto-lands on `full` (`ensureCoupleFullSharing`), so it fires for real couples. Surfacing it at `type_compatibility` (archetype + style only) *would* disclose new data — **a founder call, not ours.** Fails closed everywhere; consent gate pinned by mutation-tested regression tests (`partner-need-to-hear.test.ts`, per PRIVACY_TERMS_LIABILITY_PLAN §6.2).
     - 🔨 **Half 2 — the per-section "What This Means for You, Their Partner" layer: NOT built.** This is the remaining content lever, and unlike half 1 it *does* need the `decoded-generate-report` edge fn + regeneration of existing reports. Same rule applies: compute the cross-person comparison in code and feed it to the model as ground truth — never let it infer who-is-what.
     - ⚠️ **Spec-vs-code drift (noted 2026-07-16):** the "type + attachment only" phrasing here and in §8 predates the **2026-07-06** founder decision that removed the sharing negotiation. In shipped code the invite-link flow auto-escalates a dyad to `full` (raw scores + the full report page). The text above has been corrected to just "consent"; §8 decision #1 still carries the stale wording.
