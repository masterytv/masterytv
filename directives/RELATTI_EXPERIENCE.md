...
- **Target — restructure to relationship-first:**
  ...
  6. ⭐ **Relational interpretation per section (founder, 2026-06-26):** every section carries a **"What This Means for You, Their Partner"** sub-block that translates this person's result — and, once shared, the partner's — into *what it means for the two of you together*. The profile reads as dyad-interpretive, not two solo reports. Governed by consent; **private coaching is never surfaced.** State the privacy rule plainly in the share UI.
     - ✅ **Half 1 — the S5 reciprocal block (built 2026-07-16).** "What Your Partner Needs to Hear" had been shipping permanently empty (`phrases={[]}` hardcoded). It now fills from the partner's **own** S5 `what_you_need_to_hear` — their words, not a fresh generation about them — via `src/lib/relatti/partner-need-to-hear.ts`, and the heading personalizes to "What *Priya* Needs to Hear". Computed in **code, not the LLM**, per the dyad-interpretive rule: two independent renders would contradict each other. No report regeneration needed. Retakes propagate for free (it follows `decoded_invites.{inviter,recipient}_report_id`).
