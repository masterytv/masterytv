...
> **🔴 Gotcha — page metadata does NOT follow the brand automatically (hit live 2026-07-14, again 2026-07-15).** Next merges metadata per top-level key, so any page exporting a bare `{ title }` inherits the root layout's Mastery Coach `openGraph` + `icons` wholesale — and link-preview crawlers (iMessage/Slack) never run the client-side brand script that swaps favicons in the browser. relatti.com links previewed with the MasteryTV icon and title until every page went through `src/lib/platform/brand-metadata.ts`. **The second hit (7/15) was the sneakier variant:** `"use client"` pages can't export metadata at all, so they inherit the root head silently — relatti.com/dashboard/chat showed a "Mastery Coach" browser tab. Fix: metadata-only `layout.tsx` per client segment; the gate (`scripts/check-brand-metadata.mjs`) now fails any ROUTE with no brand-aware metadata below the root, not just bad exports. Discoverability is host-aware too: `robots.txt` + `sitemap.xml` are brand-resolving route handlers (never static files in `public/` — the old static robots.txt advertised MasteryTV on relatti.com), and indexable pages carry a `canonical` against their brand origin. Rule + patterns + crawler-style verification: **BRAND.md §15** (mandatory for every new page).
