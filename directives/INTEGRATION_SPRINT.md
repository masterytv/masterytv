...
| **I2.3** The silent seams (by hand) 🟥 ⛔ **BLOCKED on the public name** — every seam below is **brand**-keyed (`brandId === "relatti"`, `brand.id === "money"`), not program-keyed, so none of it can be written until a `BrandId` exists. And a `BrandId` cannot be added cheaply: `check:brand-tokens` demands a full identity token set in **both** themes, and 9 `Record<BrandId,…>` files compile-error at once (OG palette, sitemap, login panel, legal, notify sender, invite email, middleware, brand-metadata). That is I10 in its entirety, done blind, and then redone when the wordmark lands — the MoneyTraits rename is the precedent for what that costs. **The name is now the critical-path decision for this vertical.** | Types and gates cannot reach these: 🔥 `dashboard/page.tsx` selects with a plain `if (brandId === "relatti")` and otherwise falls through to the **executive** `DashboardHome` — add an explicit branch. Same trap class in `sidebar.tsx`, `topbar.tsx`, `settings`, and the `middleware.ts` root rewrite. Extend the `layout.tsx` inline head script's brand lists (template literal — regexes need `\\/`; never remove Next-managed `<head>` nodes). | A `?brand=integration` session renders the new brand's chrome, theme and favicon, and does **not** land on the executive dashboard. |
...
>
> Response: **`quoteFidelity()` in `_shared/output-auditor.ts`, a 14th class, hard-blocking** (`check:auditor`, now 38 cases — every case is a real shape from these runs). It tolerates line wrapping, apostrophe shape, first-letter case and trailing punctuation, and fails a substituted word, a dropped clause, an ellipsis bridge and two people stitched together. ⬜ **It is not yet wired**, for the same reason I3.4's model second-pass is not: the coach STREAMS deltas, and a draft cannot be un-sent. The auditor needs a buffered draft path, which is a decision that touches the shipped verticals. Until then the battery keeps measuring this as an **advisory** finding — reported loudly, not failed, because a prompt provably does not hold it and a permanently red battery is one nobody runs.
...
>
> **Last verified August 12, 2026** — `npm run gate` green (7/7 prompt goldens byte-identical, of which the **5 shipped verticals' goldens are unchanged**; 18 resolve cases, 56 provenance cases, 38 auditor cases, 30 tripwire, 13 trajectory, 11 memory-filter, 72 safety), and `npm run battery:integration` **3× green (63/63)** against the real pack on the real API. `npm run corpus:probe` green as of August 11. ⚠️ **Sprint 1 is UNPUSHED: 7 local commits on `staging`** (`b5fa5e8` → I4). Sprint 0 is on `origin/staging` through `dd2be6d`.
>
...
| I5 Witness turn | — | 🟢 **UNBLOCKED — this is next** | I4's pack is where I5's prompt text lives. Two of §5.3's rules landed early because they were cheap and safety-adjacent (never ask a question that presupposes content; de-pathologize as a discrete utterance), and the stage-1 layer already caps the turn at 120 words. What I5 still owns: the pre-account box (I5.1), the three-beat first-message builder (I5.2 — `coaching/first-message.ts` still writes a solo self-improvement letter), the turns-2-to-8 rules as their own battery suite (I5.3), voice input (I5.4) and the consent + 18+ gate (I5.5), which is the gate on the FIRST STORED FACT and therefore on the taxonomy migration mattering. |
| I6 The Company | — | ⬜ Blocked on I1, I5 | **Reframed August 11:** the primary delivery is a chat turn — three accounts with links, on a signal — not a page. A full reveal surface is a second-order want, not the mechanic. |
| I7 Footing check | — | ⬜ Blocked on I2 + instrument permissions | |
| I8 The Map | — | ⬜ Blocked on I6, I7 | |
| I9 Doors + triage | — | ⬜ Blocked on I10, I11 | |
| I10 Brand + theme | — | ⛔ **Blocked on the name — and it is now the critical path** | Since I2 landed, the name gates more than branding: I2.3's silent seams are all **brand**-keyed, so the dashboard/sidebar/topbar/middleware branches cannot be written without a `BrandId`, and adding one compile-errors 9 `Record<BrandId,…>` files that each need an identity decision (OG palette, sitemap, login, legal, notify sender, invite email chrome). Doing it blind means doing I10 twice — see MoneyTraits. |
...
- [ ] **Output auditor blocking the 13 banned classes** (I3.4) — *14 classes exist and are gate-tested; ⛔ **nothing calls `auditDraft` yet** — it needs a buffered draft path, because the coach streams and a delta cannot be un-sent. Top launch blocker.*
