...
>
> **One founder action is outstanding and blocks nothing else:** apply `supabase/migrations/20260812000000_integration_memory_taxonomy.sql` (widens `memory_facts_category_check` by `account`/`cost`/`disclosure`). It must land **before** any edge deploy carrying `integration-pack.ts`, because the post-processor batch-inserts and one un-admitted category drops the whole batch.
...
- [ ] Extraction taxonomy inside the live `memory_facts_category_check` — 🔥 **migration before deploy** (I4.3) — *taxonomy shipped; migration `20260812000000` written against the live constraint and **awaiting the founder's apply***
