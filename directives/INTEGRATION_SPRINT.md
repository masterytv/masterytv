...
>
> **Probe, after the refinement (built-in sample account, 588 chars):** 479–1021ms end to end · 7 of 7 claims represented in the reveal · 178 distinct accounts across the claim windows · 8/9 excerpts sentence-bounded. The boundary claim now returns *"I understood without being told that if I crossed that line, or passed through it, there would be no coming back"* — which is §5.4's promise landing. Gate: 47 provenance cases (was 23), all 5 prompt goldens byte-identical.
>
...
| I1 Corpus bridge + validation | Claude Code | 🟡 I1.1–I1.4 ✅ **live-verified + refined**, **I1.5 not started** | **Kill gate.** Bridge at `supabase/functions/_shared/corpus.ts`; rides the EXISTING coach behind the flag (no new ProgramId/brand/pack/surface). Provenance contract enforced in the return shape + `assertNoAuthoredText` (**47-case gate**, `npm run check:provenance`). **Retrieval refined August 11 (founder path A):** per-claim retrieval with round-robin merge, and excerpts re-bounded to whole sentences from `subtitles_punctuated`. **Probe green:** embedding space confirmed (self-match 1.0000), 9 accounts in **479ms**, 178 distinct across the claim windows, **7/7 claims represented**, 8/9 excerpts sentence-bounded. Findings 1 + 2 fixed; **finding 3 (analyst prose in the reveal) and the machine-translated-account question are still founder calls before I1.5 runs.** |
