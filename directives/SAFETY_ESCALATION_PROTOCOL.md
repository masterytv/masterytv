...
|---|---|---|---|
| **Tier 1 — synchronous keyword hard-stop** | Before the model replies | Pure regex (`crisis-patterns.ts`), FIRST-PERSON + EXPLICIT, high-precision. Self-harm (high/moderate) + intimate-partner abuse/coercive control (high). Moderate self-harm + all abuse get an LLM context-check (`confirmCrisisWithLLM`/`confirmAbuseWithLLM`) to cut false positives. | Third-person ("my husband wants to die"), indirect/emotional-only cues → owned by Tier 2 + the coach. |
| **Tier 2 — async LLM sweep** | After the reply streams (`EdgeRuntime.waitUntil`, zero user latency) | Haiku classifier (`safety-sweep.ts`) over the recent window. Catches self-harm, abuse, **acute distress**, and **third-party risk including child safety** (`subject_scope: third_party`). | — |

**Categories:** `self_harm`, `abuse`, `acute_distress`. **Subject scope:** `self`, `partner`, `third_party` (a child is `third_party`).
...
3. **High-severity self-harm or abuse emails a founder alert** to `tom@relatti.com`.
...
- [ ] **E15.3 remainder:** admin-access **audit logging** on reads of `crisis_flags`/messages (least-privilege + who-read-what), a documented reviewer SLA, and verify `delete-user-data` covers every content table (`messages`, `memory_facts`, `crisis_flags`, summaries). *(The isolation regression test — the other E15.3 item — is DONE, above.)*
- [ ] Confirm deployment parity: `email-inbound` and `telegram-webhook` also import `channel-router.ts` — redeploy them so those channels get the Tier-1 escalation + the `engagementId` fix.
