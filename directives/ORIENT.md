...
| **Integration coaching** — public brand **HEARD** (youheard.org), `BrandId` `heard`, slug `integration` (proposed Stage 3 — transformational/anomalous-experience) | ⭐ [INTEGRATION_SPRINT.md](INTEGRATION_SPRINT.md) (build order, Gate 3 pending) → [INTEGRATION_EXPERIENCE.md](INTEGRATION_EXPERIENCE.md) (✅ Gate 0.5 approved) → [INTEGRATION_DISCOVERY.md](INTEGRATION_DISCOVERY.md) (research + risk). Rides the Project Profound corpus (separate Supabase project). |
...
- 🔥 **A contract enforced in the tool layer is not enforced in the render layer** (found 2026-08-12 building `integration`'s I4.4, and it generalizes). `corpus.ts` proves every string it returns is byte-identical corpus text — 56 gate cases, an assertion that throws rather than returning a boolean. The guarantee stops at the model. Told character-for-character in *both* the tool payload and the system prompt, Sonnet still spliced three non-contiguous parts of one transcript into a single quotation joined by ellipses, dropped an interior clause and repaired the punctuation, and kept doing it after the rule was hardened. The same shape applies anywhere a layer hands the model verified data and the model writes the sentence the user reads: the dyad ground-truth rule below, the money report's stored numbers, any quoted excerpt. **If the rendered form matters, check the rendered form.** Fixed 2026-08-12 for `integration` only: a deterministic quote-fidelity class in `_shared/output-auditor.ts` plus a **buffered draft path** (`_shared/draft-audit.ts`), because a streamed delta cannot be un-sent. It is opt-in per pack — `CoachPack.auditDrafts`, true for integration and false for the three shipped verticals, whose streaming path is unchanged — since buffering trades the typing effect for the guarantee and that trade is only free for a coach that answers short. The path is deterministic audit → a model second pass → one regeneration → a fixed line, and it **fails open**: an auditor that can 500 the coach is worse than the drafts it catches. Two rules worth stealing for any judge: **make the judge point at the text** (a finding whose quoted span is absent from the draft is discarded, so a hallucinated violation cannot regenerate a good reply), and **run the free layer first** so a doomed draft is never paid for.
- 🔥 **A control nobody calls has no false-positive rate** (2026-08-12). `auditDraft` sat unwired for a day and looked clean; the moment it ran for real it blocked the integration pack's OWN prescribed sentence ("I'm not going to tell you it wasn't real" matched an ontological-denial pattern on its tail) and "it was real to you", which is belief in the person rather than a verdict on the world. Both would have regenerated, said the same thing again, and dumped the person on the fallback line. Fixed with sentence-scoped exemptions. **Before wiring any dormant checker, run it over the text it will actually see** — the shipped prompts, the real replies — and count what it stops.
- 🔥 **`messages.metadata` is CLOBBERED on almost every coach turn, in every vertical** (found 2026-08-12 in the first live `integration` run; it is old). The coach writes a rich metadata object on the coach message — `program`, `active_challenges`, token counts, and for integration `draft_audit` — and then `post-processor.ts` runs in `waitUntil` and does a whole-object `update({ metadata: { sentiment, topics } })`, which **replaces the JSONB column** rather than merging into it. Last writer wins and the post-processor usually does: across 60 days of coach messages, 31 of 53 relationship turns kept the coach's object and 22 kept the post-processor's — **never both** — and `draft_audit` survived **0 of 5** integration turns. So the audit trail for a blocked-and-regenerated reply does not exist in production, and neither does the `program` stamp anything downstream might read. Not fixed. Anything that writes `messages.metadata` outside the post-processor is currently writing to a column that will be overwritten seconds later; **merge, never replace**, and assume nothing you put there survives until this is fixed.
- 🔥 **A control that counts "what the person did not say" fights any feature whose job is to bring in someone else's words** (found 2026-08-12, live). The output auditor's mirroring index — the highest-leverage ban in DISCOVERY §5.3 — blocks any proper noun in a draft that is absent from the user's own text, which is exactly right for a coined entity and exactly wrong for an attributed corpus excerpt. Corpus accounts arrive as interview transcripts full of names and places, each carrying a video title and URL, so **the reveal trips `titling` by construction**: both corpus turns of the live run blocked twice and fell back to the fixed line. The general shape: a provenance control and a citation feature want opposite things about names, and the resolution is to widen the control's notion of *provably not model-authored* rather than to weaken it — corpus text already carries that proof (`assertNoAuthoredText`). Anywhere else the product quotes verified external data into a model-written sentence (dyad partner quotes, stored money numbers), expect the same collision.
- **A new PROGRAM is cheap; a new BRAND is expensive — and the measured number is 17, not 9.** Adding a `ProgramId` produced 5 bounded `Record<ProgramId,…>` errors; adding the `heard` `BrandId` on 2026-08-13 produced **17** compile errors across 12 files, and `check:brand-tokens` demands a full identity token set in both themes on top of that. A vertical can therefore be built out on the program axis (coach, safety, memory, battery) while its brand is undecided — but the dashboard/sidebar/topbar/middleware branches are **brand**-keyed and cannot be written until the name exists. See VERTICAL_PLAYBOOK §5.0.
- 🔥 **The exhaustive-Record gates do not catch a brand seam that was hoisted into a boolean** (2026-08-13, adding HEARD). `check:ternaries` looks for `=== "id" ?`, so `const isRelatti = brand.id === "relatti"` followed by `isRelatti ? a : b` passes every gate while handing each NEW brand the else-branch — it shipped MasteryTV plan names and the wrong delete-my-data copy to HEARD. Worse, the sharpest seam had no brand literal at all: HEARD's battery is deliberately empty, so a generic `requiresAssessment` nav gate locked its Coach link **forever**. **Grep for `= brand.id ===` and for gates keyed on data a new vertical will never produce**, not just for ternaries on the id.
