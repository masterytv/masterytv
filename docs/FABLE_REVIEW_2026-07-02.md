...
- **`get_auth_provider_for_email` is anon-callable** — ✅ FIXED 2026-07-09 (migration `20260709000000_lock_internal_rpcs`). It's already behind the `/api/auth/check-provider` server route (service role); anon/authenticated EXECUTE revoked, only `postgres` + `service_role` remain. Was the same PUBLIC-grant leak as the P0 (revoke had named only anon/authenticated, not PUBLIC).
- **Same-class sweep (2026-07-09, `20260709000000_lock_internal_rpcs`)** — ran the security advisor (lints 0028/0029, `anon_/authenticated_security_definer_function_executable`) across all `SECURITY DEFINER` functions. Same root cause everywhere: `REVOKE … FROM anon, authenticated` without `FROM PUBLIC`, so the default PUBLIC grant kept them callable. Locked to `service_role`: **`relatti_sync_invite`** (⚠️ highest-priority — MUTATES engagement/participant spine state; an anon caller could promote arbitrary invite_ids), plus the two triggers `handle_new_user` and `sync_is_admin_from_role` (the latter also had a mutable `search_path`, now pinned — lint 0011). Deliberately **left callable**: `get_auth_user_role` / `is_engagement_participant` (evaluated inside RLS policies as the querying role, so anon/authenticated must keep EXECUTE; both key on `auth.uid()`) and `ritual_dyad_reveal` / `ritual_submit_response` (client-called, already scoped to `authenticated`). Verified via `has_function_privilege` matrix + advisor re-run + a service-role login-path smoke test.
