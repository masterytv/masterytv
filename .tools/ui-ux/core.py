...
_GEMINI_DATA = Path.home() / ".gemini" / "antigravity" / "skills" / "ui-ux-pro-max" / "data"

# Use local copy if it has CSV files, otherwise fall back to .gemini skill path
def _find_data_dir():
    if _LOCAL_DATA.exists() and any(_LOCAL_DATA.glob("*.csv")):
        return _LOCAL_DATA
    if _GEMINI_DATA.exists():
        return _GEMINI_DATA
    return _LOCAL_DATA  # Fallback to local (will show file-not-found errors)
